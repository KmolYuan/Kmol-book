...
\renewcommand{\thechapter}{附錄\zhnum{appendix_counter}}  % 附錄\CJKnumber{\arabic{appendix_counter}}
$for(include-after)$
\newpage
$include-after$
...
性別    & $gender$ \\ \hline
出生日期 & $birth$ \\ \hline
出生縣市 & $birthplace$ \\ \hline
最高學歷 & \begin{tabular}[x]{@{}l@{}}
            $university_zh$ \\
            $institute_zh$ 碩士
        \end{tabular} \\ \hline
次高學歷 & \begin{tabular}[x]{@{}l@{}}
            $university_zh$ \\
            $institute_zh$ 學士
        \end{tabular} \\ \hline
研究    & $title_zh$ \\ \hline
\multicolumn{2}{||l||}{已發表著作} \\ \hline
$for(papers)$\multicolumn{2}{||p{0.6\paperwidth}||}{$papers$} \\ \hline$endfor$
\multicolumn{2}{||l||}{專業證照} \\ \hline
$for(licenses)$\multicolumn{2}{||p{0.6\paperwidth}||}{$licenses$} \\ \hline$endfor$
\end{tabular}
\end{center}
\vspace{\fill}
$endif$
